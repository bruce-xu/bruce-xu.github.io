...
　　仅仅使用 px 作为单位很多时候是行不通了，这就可能导致要么出现横向滚动条，要么页面留白。使用 CSS3 中的 flex 布局，外加 CSS 媒体查询，再使用 px 作为单位，倒是可以解决一些布局问题，但仍不是一个通用的方案。CSS3 中增加的 rem 单位可以弥补 px 的一些不足。rem 是相对于 html 根元素的 font-size 的，能够随着根元素字体大小的变化而变化。所以使用 rem 时，为了适配不同屏幕，需要给根元素设置合适的 font-size。
  
